  ...
  const SOCKET_URL = "http://13.219.79.56:3000";
  ...
  setTextBoxes(
    ...
    setTextBoxes(prev => {
      ...
      if (next.length === 0) setFocusedIdx(null);
      return next;
    });
  };
    ...
    prev.map((box, i) =>
      i === focusedIdx
        ? {
            ...
            fontSize: type === 'fontSize' ? value as number : box.fontSize,
            fontFamily: type === 'fontFamily' ? value as string : box.fontFamily,
          }
        : box
    )
  );
...
<input
  ...
  onChange={e => {
    ...
    // 업/다운 버튼 클릭 시에는 onChange에서 value가 항상 유효한 숫자(8~64)로 들어옴
    // 직접 입력일 때는 중간에 1, 2, '' 등 범위 밖 값도 들어올 수 있음
    const num = Number(val);
    // 업/다운 버튼 클릭인지 판별: num이 8~64 사이의 숫자이고, val이 ''(빈문자)가 아닐 때만 바로 반영
...
>
  ...
  onBlur={e => {
    // relatedTarget이 툴바 내부면 포커스 해제하지 않음
    if (
      toolbarRef.current &&
      e.relatedTarget &&
